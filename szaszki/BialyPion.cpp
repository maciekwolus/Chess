...
void BialyPion::promocja(Figura* Bierki[], int n, Sprite Obrazki[], string str)
{
	if (Bierki[n]->typ == "BPion")
	{
		Bierki[n]->grafika = Obrazki[6];
		Bierki[n]->wartosc = 9;
		Bierki[n]->typ = "BHetman";
	}
}

void BialyPion::ruch(Figura* Bierki[], string str, int n, bool& poprawnosc, int czyjRuch, int &pomocN, double tablicaBytow[8][8])
{
	if (czyjRuch % 2 == 0)
	{
		if (Bierki[n]->pierwszyRuch == 0)
		{
				...
				&& ((str[2] == Bierki[n]->x + 1 && tablicaBytow[8 - Bierki[n]->y - 1][int(Bierki[n]->x) - 96] < 0) || (str[2] == Bierki[n]->x - 1) && tablicaBytow[8 - Bierki[n]->y - 1][int(Bierki[n]->x) - 98] < 0))) //bicie na ukos
			{
				...
				&& (str[2] == Bierki[n]->x) // ten sam rzad
				&& tablicaBytow[8 - Bierki[n]->y - 1][int(Bierki[n]->x) - 97] == 0) // zsprawdzenie czy puste
			{
				Bierki[n]->x = str[2];
				...
				&& ((str[2] == Bierki[n]->x + 1 && tablicaBytow[8 - Bierki[n]->y - 1][int(Bierki[n]->x) - 96] < 0) || (str[2] == Bierki[n]->x - 1) && tablicaBytow[8 - Bierki[n]->y - 1][int(Bierki[n]->x) - 98] < 0))) //bicie na ukos
		{
			...
			(Bierki[n]->y == 5  //tylko na rzedzie 3
...
void BialyPion::szach(Figura* Bierki[], int n, bool& Szach, int c, double tablicaBytow[8][8], int i)
{
...
void BialyPion::mozliwyRuch(Figura* Bierki[], int czyjRuch, int& NBialychRuchow, int& NCzarnychRuchow, int n, string str, Sprite Obrazki[], bool& bialyMat, bool& bialyPat, bool& czarnyPat, bool& czarnyMat)
{
